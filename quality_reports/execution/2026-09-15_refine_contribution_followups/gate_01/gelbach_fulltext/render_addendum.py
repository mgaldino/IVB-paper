"""Render only the Gelbach bibliographic addendum; no scientific analysis runs."""
...
source = BASE / "gelbach_addendum.md"
output = BASE / "gelbach_addendum.pdf"
command = [
    "pandoc", str(source), "--from=markdown+tex_math_single_backslash",
    "--standalone", "--pdf-engine=xelatex", "-o", str(output),
...
(BASE / "render.log").write_text(result.stdout + result.stderr, encoding="utf-8")
...
(BASE / "build.json").write_text(json.dumps(record, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
print(output)
