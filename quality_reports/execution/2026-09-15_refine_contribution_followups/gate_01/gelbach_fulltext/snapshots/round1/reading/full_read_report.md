...
Consequentemente, continuam válidas as conclusões anteriores de que a identidade vetorial, a alocação condicional por blocos e a incorporação da covariância não constituem, sozinhas, uma contribuição distinta. O comparador forte `C+` fica mais forte depois desta leitura porque deve receber também: (i) o alvo populacional explícito; (ii) a teoria de covariância do Apêndice B; (iii) a ressalva não regular na origem; (iv) a extensão IV; e (v) a demonstração empírica de cancelamento entre componentes.
...
Encerrar a pendência documental não libera uma alegação de novidade. O conteúdo integral amplia a sobreposição conhecida e acrescenta limites que precisam entrar no comparador.

### 3.2 Alvo populacional mais forte que o registrado antes

O relatório anterior tratava corretamente a identidade como equivalência amostral, mas o artigo fornece também o alvo populacional `delta=Gamma beta_2`. `C+` deve reportar separadamente:
...
Isso reduz o espaço para afirmar que o IVB-paper torna a decomposição “estimável” ou dá a ela, pela primeira vez, um parâmetro populacional.
...
### 3.6 Extensões IV e Hausman entram no comparador

O comparador não deve ser artificialmente limitado a OLS se a alegação futura cobrir IV. A igualdade finita e a equivalência assintótica precisam respeitar identificação exata versus sobreidentificação e conjunto de instrumentos fixo. Para testes de mudança entre especificações, a existência do resultado de Hausman sob ortogonalidade deve ser reconhecida, sem convertê-lo em solução geral para inferência sobre qualquer componente ou cancelamento.
...
6. **Escopo computacional:** `L07` continua mais amplo em `b1x2`, e o artigo acrescenta a distinção teórica entre IV exatamente identificado e sobreidentificado.
...
10. **Fronteira de novidade:** `L17` muda de “pendente por falta de texto integral” para “texto integral lido; núcleo conhecido confirmado e ampliado”. A leitura não libera a novidade matemática ou inferencial.

O mapeamento linha a linha está em `claim_comparison.csv`.

## 5. Comparador `C+` após a leitura integral

O `C+` apropriado passa a ter cinco camadas explícitas.

### Camada 1: domínio e especificação

Fixar a mesma amostra, transformações, pesos, regressoras comuns, partição de blocos e matriz de instrumentos. Verificar posto. A especificação completa deve ser defendida antes de interpretar a decomposição.
...
Com essas camadas, o resultado congelado permanece: `P` e `C+` produzem os mesmos coeficientes, componentes e decisões causais quando recebem as mesmas entradas e regras. Uma diferença futura precisa vir de uma operação, garantia ou evidência adicional identificada antes do teste.
