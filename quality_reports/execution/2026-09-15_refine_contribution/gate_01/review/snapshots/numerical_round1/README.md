...
Código preparado e ainda não executado. A execução depende de revisão
...
execução se algum deles divergir.
...
3. Confronta cada projeção calculada com a expressão algébrica congelada;
   verifica sinais, inclusão, retirada, diferença dos endpoints, ações
   descritivas e certificados causais condicionais.
...
- `e2_observable_covariances.csv`: matrizes de `G_M` e `G_F` obtidas da
  transformação estrutural e dos produtos internos do fixture;
...
- `run_log.txt`, `sessionInfo.txt` e `output_manifest.csv`: log, ambiente R e
  hashes das saídas.

Qualquer falha de identidade deixa as tabelas e o log no diretório daquela
execução, marca `status=FAILED` e encerra o processo com erro.
