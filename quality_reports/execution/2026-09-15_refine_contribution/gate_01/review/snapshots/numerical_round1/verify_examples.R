...
sha256_file <- function(path) {
  output <- suppressWarnings(
    system2("shasum", c("-a", "256", shQuote(path)),
  ...
  tolower(strsplit(trimws(output), "[[:space:]]+")[[1L]][1L])
}
...
run_complete <- FALSE
on.exit({
  status_line <- if (run_complete) "status=PASS" else "status=FAILED"
  if (!file.exists(file.path(run_dir, "run_log.txt"))) {
    writeLines(enc2utf8(c(run_log, status_line)),
...
for (cell_index in seq_len(nrow(e1_cells))) {
  ...
  expected_action <- compensation_action(expected)
  p_action <- compensation_action(p$quantities)
  cp_action <- compensation_action(cplus$quantities)
  causal_action <- if (cell$cell == "E1-nulo") {
    "modelos_CET_certificados=B|C|U"
  } else {
    "modelos_CET_certificados=B"
  }
  certificates <- if (cell$cell == "E1-nulo") {
  ...
  for (procedure in c("C0", "Cplus", "P")) {
    ...
    action_rows[[length(action_rows) + 1L]] <- data.frame(
      ...
      expected_descriptive_result = if (procedure == "C0") {
        "campo_nao_reportado"
      } else {
        expected_action
      },
      ...
      descriptive_loss = if (procedure == "C0") NA_integer_ else 0L,
      causal_loss = 0L,
...
for (graph in c("G_M", "G_F")) {
  ...
  for (procedure in c("C0", "Cplus", "P")) {
    action_rows[[length(action_rows) + 1L]] <- data.frame(
      ...
      graph_set = "G_M|G_F",
      procedure = procedure,
      descriptive_result = "shift_descritivo=-1",
      expected_descriptive_result = "shift_descritivo=-1",
      causal_action = "somente_descritivo",
      conditional_certificates = paste0(
...
procedure_comparison <- data.frame(
  ...
  information_gain_claim = rep(
    "nenhum_ganho_informacional_atribuido", 5L
  ),
  stringsAsFactors = FALSE
)
...
write_text_safe(c(run_log, if (computed_pass) "status=PASS" else "status=FAILED"),
                "run_log.txt")
write_text_safe(capture.output(sessionInfo()), "sessionInfo.txt")

output_files <- list.files(run_dir, full.names = TRUE)
