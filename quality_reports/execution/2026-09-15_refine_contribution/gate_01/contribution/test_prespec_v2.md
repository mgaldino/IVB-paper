# Teste pré-especificado de contribuição, versão 2

**Estado:** revisado após comentário do coordenador, antes de qualquer implementação ou teste; pronto para congelamento antes de implementação e testes numéricos. **Data:** 15 de setembro de 2026. **Responsável pelo desenho:** `gate1_contribution`. **Âmbito:** Gate 1, núcleo OLS linear do plano aprovado. A revisão independente e a decisão global pertencem a outros responsáveis.
...
Calcula `beta_B`, `beta_C` e `d_total` em E1; calcula `beta_S`, `beta_T` e sua diferença em E2. Pode e deve aplicar o mesmo DAG e as mesmas hipóteses à decisão causal. O relatório C0 omite os componentes intermediários; isso não é uma impossibilidade informacional, pois as equações e os momentos comuns permitem calculá-los. Um analista que use essas entradas pode reconstruir a decomposição e a mesma decisão. **Não se atribuirá a C0 a regra irracional de que coeficientes semelhantes provam validade causal.** C0 representa a informação exibida pela comparação simples, não o melhor analista possível. Seu desempenho não decide a originalidade.
...
O alvo descritivo é calculado das projeções exatas. Os dois endpoints, considerados isoladamente, não identificam esse alvo; as equações e os momentos fornecidos em comum são informação suficiente para reconstruí-lo. A coluna C0 registra apenas quais campos o relatório simples exibe. Não se fará teste de significância, razão por SE de endpoint ou escolha de limiar após observar saídas.
...
Quando a informação causal é insuficiente, `somente_descritivo` acompanhado dos certificados condicionais corretos tem perda `0`. A comparação primária C+/P usa o vetor de perdas, sem pesos agregados que possam esconder erro causal. Em C0, componentes omitidos recebem `campo_nao_reportado`, sem pontuação de perda e sem alegação de erro ou incapacidade do analista. Não se atribui ganho causal à simples exibição dos componentes.
...
- Congelar este arquivo e o JSON antes da implementação. Qualquer correção científica cria nova versão com motivo e evidência já vista; as versões anteriores permanecem preservadas. Ajuste meramente tipográfico também recebe novo hash.
...
## 10. Alteração pré-teste e evidência primária adicional

A v1 foi congelada às 12:02:10 UTC. Antes de qualquer implementação/teste, o coordenador observou que C0 recebe as mesmas equações e momentos e portanto não é informacionalmente incapaz de reconstruir os componentes. A v2 mantém C0 como relatório de endpoints, exclui perda por campos não reportados e limita a comparação decisiva a C+/P. Nenhuma célula ou parâmetro foi alterado. As duas versões ficam preservadas.

Foi lida a ajuda pública do programa do próprio Gelbach, arquivada em `gate_01/literature/sources/gelbach_b1x2.sthlp:79–82,154–203,225–236`: ela documenta decomposição exata por grupos, opção robusta, cluster com dependência arbitrária intragrupo e tratamento da covariância entre parâmetros do produto. C+ já tinha essas capacidades. A ajuda não substitui a leitura integral do artigo, ainda pendente, e nenhuma seção não lida é descrita como ausente.
