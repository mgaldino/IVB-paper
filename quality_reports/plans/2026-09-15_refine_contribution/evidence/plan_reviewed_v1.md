...
**Passa quando:** existe uma diferença defensável em relação ao comparador forte, um benefício ou capacidade claramente demonstrável, e ao menos uma aplicação documentalmente viável. A leitura integral da literatura é condição para afirmar novidade. **Se falhar:** produzir um memo de decisão com opções concretas: estreitar para contribuição aplicada/pedagógica, desenvolver uma extensão específica de maior alcance ou suspender esta rota. Mudar o escopo científico é decisão do autor. **Entregáveis:** memo de contribuição, confronto com literatura, exemplos e seleção da aplicação.
...
**Passa quando:** dois leitores conseguem executar o procedimento e identificar quando ele permite interpretação causal, apenas descrição ou nenhuma estimativa confiável. Estimandos, hipóteses de dependência e tratamento da seleção estão explícitos. **Se falhar:** restringir a classe coberta; não adicionar mais métodos sem uma necessidade demonstrada. **Entregáveis:** protocolo operacional, protocolo inferencial e especificação congelada da aplicação.
...
- [ ] Para cada DGP, publicar equações, parâmetros, condições iniciais, restrições de estabilidade, burn-in, número de unidades/períodos, repetições e regras de retenção. Substituir “Task 12/13” por apêndices científicos identificados e diretamente acessíveis.
...
O caminho principal é **0 → 1 → 2 → 3 → 4 → 5 → 6 → 7**. A triagem documental da aplicação ocorre já no Gate 1. Durante a formalização, documentação e preparo estrutural de dados podem avançar sem analisar shifts ou antecipar decisões pendentes. O código da aplicação pode ser preparado durante o Gate 4, mas sua inferência final depende da validação do procedimento.

Em cada gate, o fluxo é: tarefa delimitada → implementação ou análise → congelamento → revisão independente → adjudicação → correção pelo implementador → verificação dos novos bytes. A meta é resolver a questão científica de cada etapa; a quantidade de relatórios não é um critério de sucesso.
...
Esta seção será fechada após o Devil's Advocate independente, com registro dos ataques, da adjudicação e das alterações efetivamente incorporadas. A crítica terá como objeto este plano e sua hipótese de contribuição, preservando as restrições do manuscrito atual.
