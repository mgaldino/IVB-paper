...
\end{tikzpicture}
\caption{The three elementary DAG structures. For the CET, the practical question is whether the documented causal clock makes a candidate control a fork, collider, or mediator.}
\label{fig:three_structures_pa}
\end{figure}

Forks, chains, and colliders remain the fundamental building blocks \citep{pearl2009causality, elwert2014endogenous}. What changes in TSCS work is that the same substantive variable can move across these roles as its causal timing changes. An income measure recorded before treatment begins may be a fork that blocks omitted variable bias; income measured after exposure begins may instead be a mediator or collider if treatment or the outcome process affects it.
...
The ADL + FE benchmark identifies the CET only under explicit scope conditions. Let $H_{i,t-1}$ contain the observed outcome, treatment, and covariate history needed for current assignment and the outcome process. A sufficient constant-coefficient restriction is

\[
\mathbb{E}[Y_{it}(d)\mid H_{i,t-1},\alpha_i,\tau_t]
=
\alpha_i+\tau_t+r(H_{i,t-1})'\gamma+\beta_{\mathrm{CET}}d,
\]

together with consistency, no interference, no anticipation, conditional mean exchangeability of $D_{it}$ given that history, and support for treatment after the history and fixed effects have been partialled out. The causal clock must also be recursive: $D_{it}$ precedes the contemporaneous outcome innovation, so simultaneous $Y_{it}\rightarrow D_{it}$ feedback is excluded. By contrast, $Y_{i,t-1}\rightarrow D_{it}$ feedback is compatible with identification when $Y_{i,t-1}$ and any other relevant past state are correctly included in $H_{i,t-1}$.

Under those conditions, if $\widetilde D_{it}$ is the population residual from projecting $D_{it}$ on the included history and the unit and time effects, then
...
adl_scope_table <- data.frame(
  ...
  `Scope condition` = c(
    "Consistency; no interference or a valid exposure mapping; conditional exchangeability; no unobserved contemporaneous confounding; residual treatment support",
    "No anticipation; a correctly specified observed history $H_{i,t-1}$; no simultaneous $Y_{it}\\rightarrow D_{it}$ feedback",
