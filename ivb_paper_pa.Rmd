...
task11_figure_data <- fread(
  "quality_reports/task11_dual_role_firewall/figure_2_plot_data.csv"
) %>%
  as_tibble()

task11_bias_summary <- task11_figure_data %>%
  filter(panel == "Bias relative to true beta") %>%
  group_by(model) %>%
  summarise(
    min_bias = min(bias),
    max_bias = max(bias),
...
boundary_conditions_compact <- fread(
  "quality_reports/task12_boundary_conditions_compact.csv"
) %>%
  as_tibble()

task13_main_text_summary <- fread(
...
```{r dual-role-bias-figure, fig.cap="Dual-role DGP: bias relative to the true contemporaneous treatment effect and RMSE across collider persistence. The DGP has $Z_{t-1}$ as a confounder and $Z_t$ as a contemporaneous collider; the estimand is $\\beta=1$. The displayed subset sets $\\sigma_{\\alpha_Z}=0.5$, with $N=100$, $T=30$, a burn-in of 100 periods, and 500 Monte Carlo replications per scenario. Models are TWFE short, TWFE plus $Z_{t-1}$, ADL+FE, and ADL+FE plus $Z_{t-1}$; FE models include unit and time effects. Error bars are 95\\% Monte Carlo uncertainty intervals for simulated mean bias, not estimator confidence intervals.", out.width="95%"}
knitr::include_graphics(
  "quality_reports/task11_dual_role_firewall/figure_2_dual_role_bias_rmse.pdf"
)
```

In the main dual-role design, the bias of ADL + FE + $Z_{t-1}$ stays between `r sprintf('%.3f', filter(task11_bias_summary, model == "ADL + FE + lagged Z")$min_bias)` and `r sprintf('%.3f', filter(task11_bias_summary, model == "ADL + FE + lagged Z")$max_bias)`, while the bias of TWFE + $Z_{t-1}$ remains between `r sprintf('%.3f', filter(task11_bias_summary, model == "TWFE + lagged Z")$min_bias)` and `r sprintf('%.3f', filter(task11_bias_summary, model == "TWFE + lagged Z")$max_bias)`. The static short model has the largest reported bias, reaching `r sprintf('%.3f', filter(task11_bias_summary, model == "TWFE short")$max_bias)` in the most persistent scenarios. For this DGP, the relevant comparison is therefore between specifications that represent $Z$ as a lagged state inside the dynamic outcome model and specifications that omit part of that state.

The additional existing designs delimit, rather than generalize, this comparison. Table \ref{tab:boundary_conditions_pa} records the main boundaries: each row describes a tested family of DGPs and should not be read as evidence that the conditional baseline is robust or identified outside its stated assumptions. Detailed grids, outcome measures, retained replication counts, and provenance are recorded in the static audit.

```{r boundary-conditions-table}
kable(
  boundary_conditions_compact %>% dplyr::select(
    `Boundary examined in existing DGPs`,
    `Existing evidence`,
