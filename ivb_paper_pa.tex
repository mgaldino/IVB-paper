...
specifications combine those structures across periods.
Figure\textasciitilde{}\ref{fig:dynamic_collider_pa} makes the inherited
path explicit for a contemporaneous collider. The blue arrow is the
causal path defining the CET. Conditioning on \(Z_t\) opens
...
but it does not close the contemporaneous path opened at \(Z_t\).
...
Figure\textasciitilde{}\ref{fig:dynamic_mediator_pa} represents a
different role for the same contemporaneous variable. Here \(Z_t\)
transmits part of the effect of \(D_t\) to \(Y_t\), while \(Z_{t-1}\) is
a lagged state that can affect current treatment and outcome. For the
total CET, both blue paths belong to the estimand. Conditioning on a
substantively pretreatment \(Z_{t-1}\) blocks
\(D_t\leftarrow Z_{t-1}\rightarrow Y_t\) without blocking the
within-period mediated path. Conditioning on \(Z_t\) instead blocks
\(D_t\rightarrow Z_t\rightarrow Y_t\) and changes the target to a
direct-effect parameter under additional mediation assumptions.
...
\midrule
$Z_t$ & Collider if post-treatment & Opens a backdoor path & Do not condition unless the DAG and causal clock rule out post-treatment status\\
...
The dual role is visible in
Figure\textasciitilde{}\ref{fig:dynamic_dual_role_pa}. Without
conditioning, \(D_t\leftarrow Z_{t-1}\rightarrow Y_t\) is an open
backdoor path. Conditioning on \(Z_{t-1}\) closes that fork but opens
the inherited path
\(D_t\leftarrow D_{t-1}\rightarrow Z_{t-1}\leftarrow Y_{t-1}\rightarrow Y_t\),
because \(Z_{t-1}\) is a collider on that path. Adding \(Y_{t-1}\) to
the conditioning set blocks this particular path at the noncollider
\(Y_{t-1}\). This d-separation statement is conditional on the displayed
graph: it does not close \(D_t\rightarrow Z_t\leftarrow Y_t\) if \(Z_t\)
is conditioned on, and it does not rule out other omitted paths or
finite-\(T\) estimation bias.
...
In the displayed DGP, outcome persistence transmits the inherited
collider path from \(Y_{t-1}\) to \(Y_t\).
Table\textasciitilde{}\ref{tab:firewall_pa} shows that when this edge is
removed by setting \(\rho_Y=0\), the TWFE bias in the reported design is
approximately zero. Conditioning on \(Y_{t-1}\) blocks the cited
inherited path when \(\rho_Y>0\), subject to the graph and scope
conditions stated above.

\begin{table}[!h]
\centering
\caption{\label{tab:firewall-table}Outcome persistence and the inherited collider path in the dual-role DGP. When $\rho_Y = 0$, the reported bias from conditioning on $Z_{t-1}$ is approximately zero even without $Y_{t-1}$. When $\rho_Y > 0$, the specification including $Y_{t-1}$ blocks the displayed inherited path. \label{tab:firewall_pa}}
\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\begin{tabular}[t]{rrr}
\toprule
