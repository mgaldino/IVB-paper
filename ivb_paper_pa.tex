...
benchmark reduces bias in the reported designs and preserves the
contemporaneous total effect when lagged rather than contemporaneous
controls are used. Applications illustrate how IVB quantifies
specification shifts while the benchmark provides a defensible starting
point for applied work.
\end{abstract}
...
it need not be innocuous for cumulative, delayed, or long-run effects.
The paper's positive recommendation is therefore explicitly local to the
...
\subsection{When ADL + FE Identifies the
CET}\label{when-adl-fe-identifies-the-cet}

The ADL + FE benchmark identifies the CET only under explicit scope
...
\begin{table}[!h]
\centering
\caption{\label{tab:adl-identification-estimation-table}Scope conditions for the ADL + FE benchmark: identification versus estimation and inference. \label{tab:adl_identification_estimation_pa}}
\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\begin{tabular}[t]{lll}
\toprule
...
Projection rank & $0<\mathbb{E}[\widetilde D_{it}^{2}]<\infty$ after partialling out included history and fixed effects & The population FWL coefficient is not well defined even if causal overlap holds for some contrasts\\
...
is not a general fix. In the displayed DGP, it conditions on the
persistent outcome state through which both cited lag paths reach
\(Y_t\). The practical benchmark for this dual-role design is therefore
ADL + FE with \(Y_{t-1}\) and substantively justified lags of candidate
controls, subject to the identification and estimation conditions above.
...
and role. This is exactly the use case for IVB in a paper whose applied
payoff is a benchmark rather than an automatic decision rule.
