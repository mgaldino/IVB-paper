...
The 27 full-grid cells are the Cartesian product of true ADL order `{1, 2, 3}`, `rho_D in {0.2, 0.5, 0.8}`, and direct first-lag carryover `{0, 0.25, 0.50}`. The retained panel has `N = 100` and `T = 30`; every cell receives 500 order-invariant seeded replications. The outcome-lag vectors are `(0.50)`, `(0.35, 0.25)`, and `(0.26, 0.18, 0.12)` for true ADL(1), ADL(2), and ADL(3), respectively. The lagged treatment coefficients equal `carryover * 0.5^(lag - 1)`.

The state vector is initialized from the exact stationary Gaussian distribution conditional on the unit effects. The transition covariance solves the discrete Lyapunov equation and its maximum residual must not exceed `1e-10`; only then are the common 100 burn-in periods applied. This avoids treating a spectral radius below one as sufficient evidence that an arbitrary initial condition has decayed. `initialization_manifest.csv` saves the radius, initialization method, post-stationary burn-in, and Lyapunov residual per scenario.
...
1. exact grid, parameter-range, balanced-panel, finite-value, spectral-radius, and stationary-Lyapunov invariants;
...
6. exact stationary-initialization mean/covariance identities for the most persistent scenario, plus BIC correct-lag recovery of at least 80% in each large, pre-specified base DGP (`N = 300`, `T = 60`, 20 replications, `rho_D = 0.5`, carryover `0.25`) and complete retention of the separate AIC selection frequencies;
7. refusal of an ungated full command before any data are drawn.

The stationary-initialization check establishes that the retained distribution does not inherit an arbitrary initial state even in the slowest-decaying cell. The large-base check establishes that the estimator/selection code can recover the deliberately strong DGP signal. BIC provides the recovery sanity check because its stronger complexity penalty targets exact finite-order recovery; AIC is retained separately because it can over-select dynamic lags even in a correctly specified DGP. This distinction is a pre-specified reporting rule, not a license to promote BIC as the preferred applied criterion or to filter low-recovery full-grid cells.
...
The final validation checks the raw schema, expected row count, unique scenario-replication-estimator-control keys, exact grid values, seed completeness and uniqueness, finite successful coefficients, positive cluster-robust standard errors, binary coverage values, admissible selected lags, stationary-initialization manifests, CSV readability, and pre-existing-simulation hashes. The final handoff also requires PDF openability, extracted-text inspection, and `git diff --check`.
