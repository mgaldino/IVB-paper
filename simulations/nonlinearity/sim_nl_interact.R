# ============================================================================
# sim_nl_interact.R
# NL-1b: Interaction D x H -> Z (heterogeneity in collider channel)
#
# DGP:
#   H_it ~ N(0,1)  [exogenous, iid, NOT in Y equation]
#   D_t = alpha^D_i + gamma_D Z_{t-1} + rho_D D_{t-1} + u_t
#   Y_t = alpha^Y_i + beta D_t + gamma_Y Z_{t-1} + rho_Y Y_{t-1} + e_t
#   Z_t = alpha^Z_i + delta_D D_t + delta_DH D_t H_t + delta_Y Y_t
#         + rho_Z Z_{t-1} + nu_t
#
# Researcher always estimates LINEAR models (TWFE, ADL).
# H is unobserved — researcher cannot include D x H.
# Prediction: Since E[D*H|D] = 0, average IVB may remain similar to baseline.
# The interaction adds noise to Z, potentially attenuating theta*.
#
# Grid: delta_DH x rho_Z = 10 scenarios x 500 reps
# ============================================================================
...
# ---- DGP with interaction D x H -> Z ----
sim_nl_interact <- function(N, TT, T_burn, beta, rho_Y, rho_D,
                            gamma_D, gamma_Y, delta_D, delta_Y,
                            rho_Z, sigma_aZ, delta_DH) {
  T_sim <- TT + T_burn
  ...
  for (i in 1:N) {
    ...
    for (t in 2:T_sim) {
      ...
      Z[t] <- alpha_Z[i] + delta_D * D[t] + delta_DH * D[t] * H[t] +
              delta_Y * Y[t] + rho_Z * Z[t-1] + nu
...
cat("\n")
cat(rep("=", 71), "\n", sep = "")
cat("SIM NL-INTERACT: Interaction D x H -> Z\n")
cat(rep("=", 71), "\n\n")
...
cat("     H ~ iid N(0,1), unobserved, not in Y equation\n\n")
...
for (g in 1:nrow(grid)) {
  ...
  cat(sprintf("[%d/%d] delta_DH=%.2f rho_Z=%.2f ... ", g, nrow(grid), dDH, rz))
  t0 <- proc.time()
  ...
  for (s in 1:N_REPS) {
    dt <- sim_nl_interact(P$N, P$TT, P$T_burn, P$beta, P$rho_Y, P$rho_D,
...
# Add discarded count
disc_dt <- data.table(grid, n_discarded = discarded_counts)
summ <- merge(summ, disc_dt, by = c("delta_DH", "rho_Z"), all.x = TRUE)
...
for (rz in c(0.5, 0.7)) {
  ...
  bias_tab <- s[, .(delta_DH, n_sims, n_discarded,
...
cat("\n")
cat(rep("=", 80), "\n", sep = "")
cat("F. EFFECT OF NON-LINEARITY (comparison with baseline delta_DH=0)\n")
cat(rep("=", 80), "\n\n")

baseline <- summ[delta_DH == 0, .(rho_Z,
...
cat("1. Baseline (delta_DH=0) bias should match linear simulations:\n")
print(summ[delta_DH == 0, .(rho_Z, twfe_s = round(twfe_s_bias, 4),
...
# ---- Save ----
fwrite(summ, "results/sim_nl_interact_results.csv")
fwrite(results, "results/sim_nl_interact_raw.csv")

timing <- data.table(grid, elapsed_s = elapsed_times,
                     n_discarded = discarded_counts, total_s = total_elapsed)
fwrite(timing, "results/sim_nl_interact_timing.csv")

cat(sprintf("\nResults saved to sim_nl_interact_results.csv (%d rows)\n", nrow(summ)))
cat(sprintf("Raw saved to sim_nl_interact_raw.csv (%d rows)\n", nrow(results)))
cat(sprintf("Timing saved (total %.1fs)\n", total_elapsed))

writeLines(capture.output(sessionInfo()),
           "results/sim_nl_interact_sessioninfo.txt")
