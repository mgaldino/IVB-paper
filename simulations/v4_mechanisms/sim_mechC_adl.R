...
#
# Grid: prob_switch x delta_D x delta_Y x rho_Z = 4x2x3x2 = 48 scenarios x 500 reps
...
sim_binary_dynamic <- function(N, TT, T_burn, beta, rho_Y,
  ...
  is_switcher <- unit_type == "switcher"
  t_star[is_switcher] <- sample((T_burn + 2):T_sim, sum(is_switcher), replace = TRUE)
...
est_binary_models <- function(dt) {
  tryCatch({
    # Collinearity pre-check: within-unit correlation of D and D_lag
    # With binary staggered D, these differ only at the switch period.
    D_dm  <- dt[, D - mean(D), by = id_f]$V1
    Dl_dm <- dt[, D_lag - mean(D_lag), by = id_f]$V1
    cor_DDlag <- cor(D_dm, Dl_dm, use = "complete.obs")
    # If near-perfect collinearity (|r| > 0.999), discard rep
    if (is.na(cor_DDlag) || abs(cor_DDlag) > 0.999) return(NULL)
    ...
    # Check: if D_lag was dropped by fixest due to collinearity, discard rep
...
# ---- Grid: 4 x 2 x 3 x 2 = 48 scenarios ----
# delta_Y=0: pure collider (no Y->Z), baseline for comparison
grid <- CJ(
...
# Stationarity check: reduced-form VAR(1) for (Y_t, Z_t).
# Substituting Y_t into the Z equation gives Z_t as function of (Y_{t-1}, Z_{t-1}):
#   Z_t = delta_Y*(rho_Y*Y_{t-1} + gamma_Y*Z_{t-1} + ...) + rho_Z*Z_{t-1} + ...
# Companion matrix eigenvalues must all be < 1 in modulus.
cat("Stationarity check (VAR eigenvalues):\n")
for (g in 1:nrow(grid)) {
  dY <- grid$delta_Y[g]
  rz <- grid$rho_Z[g]
  # Companion matrix of reduced-form VAR(1) for (Y_t, Z_t):
  #   Y_t = rho_Y * Y_{t-1} + gamma_Y * Z_{t-1} + ...
  #   Z_t = delta_Y*rho_Y * Y_{t-1} + (rho_Z + delta_Y*gamma_Y) * Z_{t-1} + ...
  A <- matrix(c(P$rho_Y, P$gamma_Y,
                dY * P$rho_Y, rz + dY * P$gamma_Y), nrow = 2, byrow = TRUE)
  max_eig <- max(abs(eigen(A, only.values = TRUE)$values))
  if (max_eig >= 1) {
    stop(sprintf("Unstable DGP at grid row %d: delta_Y=%.1f, rho_Z=%.1f, max|eig|=%.3f",
                 g, dY, rz, max_eig))
  }
}
...
# This is an exact identity per-rep, so discrepancy should be < floating-point tolerance
if (nrow(results) > 0) {
...
# Aggregated check (small discrepancy from Jensen's: E[-theta*pi] != -E[theta]*E[pi])
...
# 3. ADL_all dominates TWFE_s in |bias| (key claim of the paper)
n_adl_wins <- summ[, sum(abs(adl_all_bias) < abs(twfe_s_bias))]
cat(sprintf("3. ADL_all |bias| < TWFE_s |bias|: %d/%d scenarios\n", n_adl_wins, nrow(summ)))
cat(sprintf("   ADL_all bias range: [%.4f, %.4f]\n", min(summ$adl_all_bias), max(summ$adl_all_bias)))
...
# ---- Save ----
fwrite(results, "results/sim_mechC_adl_raw.csv")
fwrite(summ, "results/sim_mechC_adl_results.csv")

timing <- data.table(
  step = "total", elapsed_sec = round(elapsed, 1),
  n_scenarios = nrow(grid), n_reps = N_REPS, n_workers = 4L
)
fwrite(timing, "results/sim_mechC_adl_timing.csv")

writeLines(c("sim_mechC_adl.R", format(Sys.time()), "",
             capture.output(sessionInfo())),
           "results/sim_mechC_adl_sessioninfo.txt")

cat("\nDone. Files saved:\n")
cat("  results/sim_mechC_adl_raw.csv\n")
cat("  results/sim_mechC_adl_results.csv\n")
cat("  results/sim_mechC_adl_timing.csv\n")
cat("  results/sim_mechC_adl_sessioninfo.txt\n")
