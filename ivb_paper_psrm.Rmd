...
Directed Acyclic Graphs (DAGs) provide a clear, qualitative answer to the question of *whether* including a particular variable introduces bias \citep{pearl2009causality, imbens2020potential, montgomery2018conditioning, blackwell2018make}. However, they are silent on the equally important question of *how much* bias results. A researcher who discovers that a control variable is a collider still does not know the magnitude or direction of the bias without additional tools.
...
\label{eq:ivb_main}
\end{equation}
where $\theta^{\star}$ is the coefficient on the collider in the misspecified (``long'') regression and $\pi$ is the coefficient on the treatment in an auxiliary regression of the collider on the treatment and legitimate controls.^[In the cross-sectional derivation, $\theta^{\star}$ corresponds to $\beta_2^{\star}$ and $\pi$ corresponds to $\phi_1$; we use the TSCS notation throughout the paper as it is the more general form.] Both quantities are directly estimable from data, making the formula immediately operational for applied researchers.

The IVB formula mirrors the well-known Omitted Variable Bias (OVB) formula, but addresses the symmetrically opposite problem. Where OVB quantifies the cost of leaving out a relevant confounder, IVB quantifies the cost of including a collider. We show that this result holds not only in standard cross-sectional regression but extends naturally to the Autoregressive Distributed Lag (ADL) models and Two-Way Fixed Effects that are the workhorse of TSCS analysis. The extension relies on the Frisch--Waugh--Lovell (FWL) theorem \citep{frisch1933partial, lovell1963seasonal} and yields the same functional form, with the key quantities now partialled out by the dynamic controls (lagged dependent variables and lagged treatments or fixed effects).

Beyond the formula itself, we investigate the structural conditions under which IVB is large or small in panel settings with fixed effects. We show that when the treatment--collider association is primarily cross-sectional (e.g., richer countries have better governance in levels), fixed effects remove this association, and the residual within-unit IVB is small. Additionally, binary treatments with few switchers inflate standard errors relative to a constant population IVB, further reducing the bias in standard-error units.
...
The *control checking* heuristic includes causes of the outcome to improve precision \citep[e.g.,][]{dietrich2016donor}; *confounding checking* targets joint causes of treatment and outcome to block omitted variable bias \citep[e.g.,][]{hegre2001toward}. Neither asks whether a candidate control is a common *effect* of treatment and outcome---a collider whose inclusion opens rather than closes backdoor paths \citep{cinelli2021crash}. In TSCS settings, this blind spot is compounded by the temporal structure of the data, which creates new opportunities for collider bias absent in pure cross-sections.
...
In this section, we derive the Included Variable Bias (IVB) formula for the bias introduced by conditioning on a collider variable. We begin with the cross-sectional case. While the derivation is quite simple and a direct application of anatomy of regression theorem, it allows the clearest comparison to the classical OVB formula, and a clean extension of the result to ADL models used in TSCS analysis.
...
\label{eq:twfe_short}
\end{equation}
and the misspecified model including a collider:
\begin{equation}
...
A sharper result obtains when the treatment is autoregressive and the collider depends only on the contemporaneous treatment:
...
\noindent \textit{Proof.} With $\pi_\ell = 0$ for $\ell \geq 1$, the general formula reduces to $\text{IVB}(k) = -\theta^{\star} \pi_0 \omega_{0k}$. Under AR(1) treatment, $\omega_{0k} = \text{Cov}(\widetilde{D}_t, \widetilde{D}_{t-k}) / \text{Var}(\widetilde{D}_{t-k}) = \rho_D^k$. The coefficient $\theta^{\star}$ is stable across $k$ because $D_{t-k}$ is a weak predictor of $Z_t$ when $k$ is large, so partitioning on $D_{t-k}$ barely affects the projection of $Y$ on $Z$. \hfill $\square$

The corollary has immediate practical implications. For highly persistent treatments---such as democracy indices or GDP per capita, where $\rho_D \approx 0.95$---the bias decays slowly: $\text{IVB}(5) \approx 0.77 \times \text{IVB}(0)$. By contrast, for treatments with moderate autocorrelation ($\rho_D \approx 0.5$), the decay is rapid: $\text{IVB}(3) \approx 0.13 \times \text{IVB}(0)$. A useful rule of thumb is that $k^{\star} \approx \log(\text{SE}/|\text{IVB}(0)|) / \log(\rho_D)$ lags are needed for IVB to fall below one standard error of $\hat{\beta}$. An important limitation is that condition (ii) is restrictive: if $Z$ depends on lags of $D$ (i.e., $\pi_\ell \neq 0$ for some $\ell \geq 1$), the decay is governed by the weighted sum in the general formula and can be substantially slower. In the six empirical applications below, all treatments are highly persistent, which explains why lagging alone does not resolve the collider problem.
...
We discuss five structural features of typical TSCS designs that help explain why empirical IVBs tend to be modest. The first follows directly from the algebra of Corollary~\ref{cor:ivb_twfe}. The remaining four are documented with Monte Carlo simulations spanning over 300 DGP configurations and 500 replications each.

## Fixed Effects Absorb the Between-Unit Collider Channels \label{sec:mech_ab}

As shown in Corollary~\ref{cor:ivb_twfe}, the IVB under TWFE depends only on the within-unit components of $\theta^{\star}$ and $\pi$. If the association between $D$ and $Z$ is primarily cross-sectional (e.g., richer countries have better governance in levels), unit fixed effects absorb this association and $\pi$ is small. An analogous argument applies to $\theta^{\star}$: if the outcome--collider association is driven by persistent cross-country differences rather than year-to-year fluctuations, fixed effects remove it. In many TSCS applications in comparative politics, this is precisely the case: treatment variables like democratization or peacekeeping deployment vary primarily across countries in levels, and their within-country variation is modest.

## Few Switchers Inflate Standard Errors Relative to the IVB \label{sec:mech_c}

In many TSCS studies, the treatment is binary and slow-moving: democratization, peacekeeping deployment, policy adoption. Most units are either always-treated or never-treated, and only a small fraction of ``switchers'' change treatment status during the observation period. With fewer switchers, the standard error of $\hat\beta$ increases sharply, while the population IVB remains roughly constant. The ratio $|\text{IVB}/\text{SE}(\hat\beta)|$ therefore \textit{decreases} as the number of switchers falls. In Monte Carlo simulations ($N = 200$, $T = 30$, $\beta = 1$, 500 replications), reducing the fraction of switchers from 70\% to 10\% increases the standard error by a factor of 2--3, reducing $|\text{IVB}/\text{SE}|$ proportionally.

An important caveat: the fact that $|\text{IVB}/\text{SE}|$ is small does not mean the bias is harmless. The population IVB is constant regardless of the number of switchers. In a study with few switchers and a marginally significant result, the IVB could still be large enough relative to $\hat\beta$ to flip the sign or significance of the treatment effect, even though $|\text{IVB}/\text{SE}| < 1$. The researcher should therefore consider both $|\text{IVB}/\hat\beta|$ (the bias as a share of the estimated effect) and $|\text{IVB}/\text{SE}|$ (the bias relative to sampling uncertainty).

## ADL Specifications Reduce Residual IVB \label{sec:mech_adl}

The preceding mechanisms explain why IVB is modest under TWFE. But researchers can further reduce bias by adopting an Autoregressive Distributed Lag (ADL) specification that includes lagged dependent and independent variables, as recommended by \citet{imai_kim2021} for panel data with causal dynamics.

To quantify this gain, we simulate a DGP with exogenous binary treatment (staggered adoption) and a $Z$ variable that is both a confounder ($Z_{t-1} \to Y_t$) and a collider ($D_t \to Z_t$, $Y_t \to Z_t$) with persistence ($Z_{t-1} \to Z_t$). We estimate nine specifications ranging from TWFE without controls to ADL models with all available lags ($Y_{t-1}$, $D_{t-1}$, $Z_{t-1}$). Across 48 scenarios ($N = 100$, $T = 30$, 500 replications), TWFE without $Z$ exhibits bias of 77--169\% of $\beta$---driven by omitted variable bias from the confounding channel of $Z$. The ADL specification including $Y_{t-1}$, $D_{t-1}$, and $Z_{t-1}$ reduces bias to less than 3\% of $\beta$ in every scenario. Including the lagged dependent variable $Y_{t-1}$ is the key: it blocks the collider path $D_t \to Z_t \leftarrow Y_t$ that would otherwise transmit IVB through $Z$'s persistence, while the lagged controls absorb the confounding channel. This result is consistent with the identification strategy in \citet{imai_kim2021}, where the ADL model achieves sequential ignorability under weaker assumptions than TWFE.

## Robustness to Functional-Form Misspecification \label{sec:mech_nl}

The IVB formula is derived under linearity. If the collider channel $D \to Z$ or $Y \to Z$ is nonlinear, the linear ADL model is misspecified, and the FWL decomposition is only approximate. To assess the practical relevance of this concern, we simulate three classes of nonlinearity in the collider equation: (i)~concave transformations ($\log(1 + |D|)$, soft-clamped polynomial), (ii)~polynomial terms ($D^2$), and (iii)~interaction terms ($D \times H$ with $H$ exogenous). Each is combined with the baseline dual-role DGP across varying nonlinearity strengths and collider persistence levels.

For bounded nonlinearities---concave or saturating transformations that are realistic for most political science applications---the ADL model with all lags remains robust: bias stays below 3\% of $\beta$, comparable to the linear baseline. TWFE bias increases modestly relative to the linear case but remains within the same order of magnitude. For unbounded nonlinearities (e.g., high-degree polynomials with large coefficients), the DGP can become explosive, and bias grows substantially. However, such configurations are empirically implausible: they require the collider to respond without bound to marginal increases in the treatment, which contradicts the ceiling and floor effects that characterize most real-world variables.

## Feedback from Outcomes to Treatment \label{sec:mech_feedback}

A more fundamental threat arises when the outcome causally affects future treatment: $Y_{t-1} \to D_t$. This feedback violates the strict exogeneity assumption that underpins static TWFE \citep{imai_kim2021} and introduces a direct endogeneity channel that is distinct from the collider mechanism.

We simulate this scenario by adding a feedback parameter $\phi$ to the treatment equation: $D_{it} = \alpha^D_i + \phi Y_{i,t-1} + \rho_D D_{i,t-1} + \lambda_D Z_{i,t-1} + u_{it}$, with $\phi \in \{0, 0.05, 0.1, 0.15\}$ and the dual-role $Z$ DGP otherwise unchanged ($N = 100$, $T = 30$, 500 replications). TWFE bias grows monotonically with $\phi$: from 43\% of $\beta$ at $\phi = 0$ to 80\% at $\phi = 0.15$ (with $\rho_Z = 0.5$). The ADL specification with all lags, however, remains remarkably robust: $|\text{bias}| \leq 1\%$ of $\beta$ across all stable scenarios, because $Y_{t-1}$ in the regression absorbs the feedback channel. For $\phi \geq 0.18$ (approximately), the joint dynamic system $(D, Y, Z)$ becomes explosive---the companion matrix of the reduced-form VAR has eigenvalues exceeding unity---which marks an analytical boundary beyond which the DGP is non-stationary.

This result provides both reassurance and a boundary condition. When feedback is moderate ($\phi \leq 0.15$), ADL with fixed effects effectively absorbs the endogeneity, and IVB remains negligible. When feedback is strong enough to destabilize the system, no OLS-based estimator is adequate, and researchers should consider GMM or instrumental variable approaches.

## Summary: When Is IVB Small?

In applications typical of comparative politics and international relations---TWFE panels with binary treatments and slow-moving variables---five mechanisms operate together to keep IVB small. Fixed effects absorb the between-unit collider channels, leaving only within-unit co-movement to drive the IVB. Few switchers inflate standard errors relative to a constant population IVB, making the bias small in standard-error units. ADL specifications with lagged controls reduce residual bias to less than 3\% of $\beta$ by blocking collider paths through the lagged dependent variable. Bounded nonlinearities in the collider channel---the empirically relevant case---do not qualitatively change the IVB picture. Finally, moderate feedback from outcomes to treatment ($Y_{t-1} \to D_t$) is absorbed by the ADL specification, with IVB remaining at approximately 1\% of $\beta$.

These features characterize the majority of TSCS designs in comparative politics, which helps explain why empirical IVBs are often modest. The boundary conditions are also informative: unbounded nonlinearities in the collider channel and strong outcome-to-treatment feedback that destabilizes the dynamic system can generate substantial bias. Researchers using cross-sectional data, short panels, continuous treatments, or settings with strong feedback loops should expect potentially larger IVBs.

These observations are consistent with the analytical results of \citet{ding2015adjust}, who showed in a cross-sectional LSEM that pure M-Bias is a ``higher-order'' effect---a product of four structural correlations---that tends to be small unless all four are simultaneously large. The empirical applications in the next section confirm this pattern.
...
## Appendix C: Simulation Code Description \label{app:code}
...
## Appendix D: Variance of the IVB Estimator \label{app:variance}
...
## Appendix E: Control Classification and Full IVB Results \label{app:classification}
...
Table~\ref{tab:ivb-full-table} reports the complete IVB decomposition for all 57 study-control combinations, regardless of collider plausibility. This table is provided for completeness and transparency; the substantive interpretation of any individual IVB as collider bias requires the causal reasoning documented in the classification tables above.
