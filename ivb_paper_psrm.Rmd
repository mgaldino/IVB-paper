...
The remainder of this paper is organized as follows. Section~\ref{sec:control} reviews the standard heuristics for selecting control variables and identifies their limitations. Section~\ref{sec:dags} introduces DAGs and illustrates collider bias in a TSCS context using a running example from the civil war literature. Section~\ref{sec:ivb} presents the core theoretical contribution: the derivation of the IVB formula for cross-sectional and ADL models, including a result showing that replacing the treatment with a longer lag does not mechanically eliminate collider bias. Section~\ref{sec:montecarlo} validates the formula through Monte Carlo simulations. Section~\ref{sec:application} provides a practical walkthrough of the formula using the civil war example. Section~\ref{sec:conclusion} concludes with recommendations for applied researchers.
