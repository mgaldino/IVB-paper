...
We discuss three structural features of typical TSCS designs that help explain why empirical IVBs tend to be modest. The first two follow directly from the algebra of FWL projection under fixed effects; the third is documented with Monte Carlo simulations.

## Fixed Effects Absorb the Between-Unit Collider Channels \label{sec:mech_ab}

The IVB is $-\hat\theta^{\star} \hat\pi$, where $\hat\pi$ comes from the auxiliary regression $Z \sim D + \text{FE}$ and $\hat\theta^{\star}$ from the long model. Under TWFE, unit fixed effects absorb any time-invariant component of the association between $D$ and $Z$ or between $Y$ and $Z$. This is a standard property of within-estimation, but its implication for IVB is worth stating explicitly.

Suppose the collider depends on the treatment through both a between-unit and a within-unit channel: $Z_{it} = \gamma_D^{\text{btw}} \mu_i^D + \gamma_D^{\text{wth}} (D_{it} - \mu_i^D) + \gamma_Y Y_{it} + \eta_i + \mu_t + \nu_{it}$. Because $\gamma_D^{\text{btw}} \mu_i^D$ is collinear with the unit fixed effect $\eta_i$, it is absorbed, and the surviving auxiliary coefficient is $\pi = \gamma_D^{\text{wth}} + \gamma_Y \beta$---independent of $\gamma_D^{\text{btw}}$. An analogous argument applies to the $Y \to Z$ channel: $\theta^{\star}$ depends only on the within-unit association between $Y$ and $Z$, not on persistent cross-country differences.

The practical implication is that even if $D$ and $Z$ are strongly correlated in the raw data, the IVB can be small if that correlation is primarily cross-sectional rather than temporal. In many TSCS applications in comparative politics, this is precisely the case: treatment variables like democratization or peacekeeping deployment vary primarily across countries in levels, and their within-country variation is modest.
...
# ---- Collider candidates identified via DAG reasoning (see Appendix G) ----
...
## Appendix F: Variance of the IVB Estimator \label{app:variance}
...
## Appendix G: Control Classification and Full IVB Results \label{app:classification}
