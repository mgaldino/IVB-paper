...
The IVB formula mirrors the well-known Omitted Variable Bias (OVB) formula, but addresses the symmetrically opposite problem. Where OVB quantifies the cost of leaving out a relevant confounder, IVB quantifies the cost of putting a collider. We show that this result holds not only in standard cross-sectional regression but extends naturally to the Autoregressive Distributed Lag (ADL) models and Two-Way Fixed Effects that are the workhorse of TSCS analysis. The extension relies on the Frisch--Waugh--Lovell (FWL) theorem \citep{frisch1933partial, lovell1963seasonal} and yields the same functional form, with the key quantities now partialled out by the dynamic controls (lagged dependent variables and lagged treatments or fixed effects).

Beyond the formula itself, we investigate the structural conditions under which IVB is large or small in panel settings with fixed effects. We show that is primarily cross-sectional (e.g., richer countries have better governance in levels), fixed effects remove this association, and the residual within-unit IVB is small. Additionally, binary treatments with few switchers inflate standard errors relative to a constant population IVB, further reducing the bias in standard-error units.
...
We discuss two structural features of typical TSCS designs that help explain why empirical IVBs tend to be modest. The first follows directly from the algebra of Corollary~\ref{cor:ivb_twfe}; the second is documented with Monte Carlo simulations.
...
In applications typical of comparative politics and international relations---TWFE panels with binary treatments and slow-moving variables---two mechanisms operate together. Fixed effects absorb the between-unit collider channels, leaving only within-unit co-movement to drive the IVB. Few switchers inflate standard errors relative to a constant population IVB, making the bias small in standard-error units. These features characterize the majority of TSCS designs in comparative politics, which helps explain why empirical IVBs are often modest. Researchers using cross-sectional data, short panels, or continuous treatments should expect potentially larger IVBs.
